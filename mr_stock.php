...
<html>
  ...
  <link href="asset/css2.css" rel="stylesheet">
	...
	<body>
		...
		<h1 class="h1 text-primary text-lg text-center">Add Product in M.R. Stock
...
<br />
    <h3 class="h3 text-primary">Stock Details</h3>
    ...
    <td><a href="mr_stock.php?edit=<?php echo $row['Id'];?>" class="btn btn-info"><i class='fas fa-edit'></i></td>
    <td><a href="delete.php?delete=<?php echo $row['Id'];?>" class="btn btn-danger"><i class='far fa-trash-alt'></i></a></td>
   <?php
    echo "</tr>";
    }   
    echo "</table>";
  ...
  echo "<table class='container text-center table table-bordered'>
    ...
    <?php if($update == true) { ?>
        ...
        <h3 class="h3 text-primary">Update Product in Stock
        <small class="text-muted">Update your product in stock here..</small>
    </h3>
    <?php } else {?>
    <h3 class="h3 text-primary">Add Product in Stock
  ...
  ?>
    ...
    <form method="POST" name="mrform" action="mr_stock.php">
    ...
    <th>Retail Price</th>
        ...
        <tr>
          <td>
          <input type="text" name="companyName"  value="<?php echo $cname; ?>" class="form-control text-center" placeholder="Compmay Name" autocomplete="off"required />
          </td>
        
          <td>        
          <input type="text" name="tabletName"  value="<?php echo $tname; ?>" class="form-control text-center" placeholder=" Tablet Name" autocomplete="off"required />
          </td>

          <td>        
          <input type="text" name="tabletDiscription"  value="<?php echo $tdisp; ?>" class="form-control text-center" placeholder=" Tablet Discription" autocomplete="off"required />
          </td>

          <td>
          <input type="number" name="quantity" value="<?php echo $qty; ?>" class="form-control text-center" placeholder="Tablet Quantity" autocomplete="off" min = "0" max="<?php echo $_SESSION['quantity'];?>" required/>
          </td>
          <td>
          <input type="date" name="mfDate"  value="<?php echo $MFD; ?>" class="form-control text-center" placeholder="Manufacture Date" autocomplete="off" required/>
          </td>
          <td>
          <input type="date" name="exDate"  value="<?php echo $EXD; ?>" class="form-control text-center" placeholder="Expairy Date" autocomplete="off" required/>
          </td>
          <td>
          <input type="number" name="holsalePrice" value="<?php echo $WSP; ?>" class="form-control text-center" placeholder="Holesale Price"  autocomplete="off"min = "0" required/>
          </td>
          <td>
          <input type="number" name="retailPrice" value="<?php echo $REP; ?>" class="form-control text-center" placeholder="Retail Price"  autocomplete="off"min = "0" required/>
          </td>
        </tr>
      </table>
        <?php if($update == true) {?>
          <button type="submit" name="update" class="btn btn-info" style="margin-left: 10px;">
          Update</button>
          <?php } else {
          ?>
    ...
    <br />
    <h2 class="text-center"><a class="btn btn-outline-primary btn-lg" href="logout.php">Logout</a></h2>
